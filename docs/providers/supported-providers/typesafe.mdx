...
<Note>
Success responses are byte-compatible with TypeSafe's own API. Error responses preserve the upstream status code and message but are wrapped in Bifrost's error envelope, and requests rejected by Bifrost's local validation return `400` where TypeSafe's own validation would return `422`.
</Note>

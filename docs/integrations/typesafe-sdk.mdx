...
- **Success responses are byte-compatible** with TypeSafe's own API: native `answers` with `noul`/`choice`/`score` value fields and `input_tokens`/`output_tokens` usage.
...
- **Errors**: upstream status codes (401, 422, 429, 529) and messages are preserved but wrapped in Bifrost's error envelope, and requests rejected by Bifrost's local validation return `400` where TypeSafe's own validation would return `422`.

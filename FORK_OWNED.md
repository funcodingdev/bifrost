...
| `README.md` | **generated** — fork header + upstream's README body | 4 |

The first three are additive and low-churn; a conflict is a one-line re-apply.

`README.md` is generated by `.fork/render-readme.sh`: a fork-owned Chinese
header, then upstream's own README taken verbatim from the tag being merged.
Only the part above the rule belongs to this fork, so upstream's README keeps
improving with upstream instead of decaying into a stale translated snapshot.

Because it is generated, `.gitattributes` marks it `merge=ours` — a conflict in
a generated file is noise, and the sync re-renders it from the newly merged
upstream README immediately afterwards. That attribute is inert without
`git config merge.ours.driver true`, which the sync workflow sets.
`README.en.md` is a new file and never conflicts.

To re-render by hand after editing a header:

```bash
.fork/render-readme.sh upstream/main
```
...
.fork/readme-header.zh.md  the Chinese half of README.md
.fork/readme-header.en.md  the English half of README.en.md
.fork/render-readme.sh     composes both from upstream's README

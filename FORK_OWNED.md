...
| --- | --- | --- |
| `ui/vite.config.mts` | imports `autoI18n()`; aliases `@/components/themeToggle` to the fork override | 1 |
...
| `README.md` | replaced with a Chinese translation of upstream's | 4 |

The first three are additive and low-churn; a conflict is a one-line re-apply.

`README.md` is a full Chinese translation of upstream's README with the install
commands swapped for this fork's image. `README.en.md` is upstream's English
with the same swap, and is a new file that never conflicts.

Because `README.md` is fully rewritten, an upstream edit would conflict every
time — about four times a year, each one halting an unattended sync before it
can publish. `.gitattributes` marks it `merge=ours` so those resolve in our
favour automatically. The attribute is inert without
`git config merge.ours.driver true`, which the sync workflow sets.

Taking ours means upstream's README edits do not arrive on their own, so the
sync hashes upstream's README and compares it with `.fork/upstream-readme.sha256`.
On a change it opens an issue asking for the translation to be ported, and
records the new hash so the same change is only ever raised once. The release
still publishes — a stale paragraph in a README is not a reason to withhold an
image.
...
ui/i18n/runtime.ts      __t / __tx, locale detection and switching
ui/i18n/components/localeToggle.tsx   the language switcher
ui/i18n/overrides/themeToggle.tsx     renders it beside upstream's theme toggle
...
.fork/upstream-readme.sha256  fingerprint of the upstream README last ported
...
### The language switcher, and why it is an alias

The switcher has to sit in the topbar, and `ui/components/topbar.tsx` is
upstream's highest-churn UI file — 14 commits in 90 days. Editing it would put a
conflict on the critical path of nearly every sync for one line of JSX.

So `vite.config.mts` aliases the specifier `@/components/themeToggle` to
`ui/i18n/overrides/themeToggle.tsx`, which renders `<LocaleToggle/>` next to
upstream's own toggle. `topbar.tsx` keeps its original import and is never
touched. The override reaches the real module by a relative path, because alias
matching works on the specifier and an aliased import would loop back.

The alias entry must precede the generic `"@"` key: alias keys match by prefix
and the first hit wins.

`tsc` does not see any of this — it resolves the alias to the original module —
so `tools/i18n/override.test.mjs` carries the assertions instead: that the
override mirrors every upstream export, that it imports relatively, and that
`topbar.tsx` still imports `ThemeToggle` through the aliased specifier. Without
that last one, upstream restructuring the topbar would make the switcher vanish
silently rather than fail.

Locale selection follows `navigator.language` on first load and persists an
explicit choice, so the localised build opens in Chinese for a Chinese browser
